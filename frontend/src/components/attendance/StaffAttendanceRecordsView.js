...
export default function StaffAttendanceRecordsView({
  title = 'Staff Attendance',
  description = 'View staff/teacher attendance records.',
  roleFilter,
} = {}) {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetchStaffAttendance({ role: roleFilter, fromDate: range.fromDate, toDate: range.toDate })
  ...
  async function handleExport() {
    ...
    try {
      const range = getRange({ mode, month, year, fromDate, toDate })
      const blob = await exportStaffAttendance({
        role: roleFilter,
      ...
      link.href = url
      link.download = `staff_attendance_${range.fromDate}_to_${range.toDate}.csv`
  ...
  const attendanceRate = summary.total ? ((summary.present / summary.total) * 100).toFixed(1) : '0.0'
  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-4">
        ...
        <AttendanceKpiGrid
          items={[
            { label: 'Total Days', value: summary.total, tone: 'blue', hint: `${mode === 'month' ? 'Monthly' : mode === 'year' ? 'Yearly' : 'Custom'} range` },
            { label: 'Present', value: summary.present, tone: 'green', hint: `${attendanceRate}% attendance rate` },
            { label: 'Absent', value: summary.absent, tone: 'red', hint: 'Not present' },
            { label: 'Late / Leave', value: summary.late + summary.leave, tone: 'amber', hint: 'Late + leave days' },
          ]}
        />

...
function getCreateButtonLabel(section, editingId) {
  if (section === "categories")
    return editingId ? "Update Category" : "Create Category";
  if (section === "items") return editingId ? "Update Item" : "Create Item";
  if (section === "stock")
    return editingId ? "Update Movement" : "Record Movement";
  if (section === "distribution")
    return editingId ? "Update Distribution" : "Issue Item";
  return "Save";
}
...
export default function InventoryWorkspace({ section = "overview" }) {
  ...
  async function loadData(nextFilters = filters) {
    ...
    try {
      ...
      const [categoryRows, itemRows, sectionRows] = await Promise.all([
        categoryPromise,
        itemPromise,
        sectionPromise,
      ]);
      ...
      setError(
        e?.response?.data?.error ||
          `Failed to load ${getRecordLabel(section)}s`,
      );
    } finally {
      setLoading(false);
    }
    ...
    if (section === "stock") {
      setForm({
        ...
        movementDate: row.movementDate
          ? new Date(row.movementDate).toISOString().slice(0, 10)
    ...
    if (section === "distribution") {
      setForm({
        ...
        distributedAt: row.distributedAt
          ? new Date(row.distributedAt).toISOString().slice(0, 10)
          : new Date().toISOString().slice(0, 10),
        note: row.note || "",
      });
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(
        e?.response?.data?.error || `Failed to save ${getRecordLabel(section)}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(row) {
    const label =
      row.name || row.title || row.recipientName || row.reference || "record";
    ...
    try {
      ...
      if (section === "distribution")
        await deleteInventoryDistribution(row._id);
      ...
      setError(
        e?.response?.data?.error ||
          `Failed to delete ${getRecordLabel(section)}`,
      );
    } finally {
      setSaving(false);
    }
          ...
          {[
            {
              ...
              title: "Categories",
              text: "Manage category groups for stock items.",
            },
            {
              href: "/admin/inventory/items",
              title: "Items",
              text: "Create and maintain inventory items and quantities.",
            },
            {
              href: "/admin/inventory/stock",
              title: "Stock",
              text: "Record item inflow and outflow movements.",
            },
            {
              href: "/admin/inventory/distribution",
              title: "Distribution",
              text: "Track issued items and recipient details.",
            },
          ].map((card) => (
            <Link key={card.href} href={card.href} className=" p-5 rounded-lg border border-[--color-primary] text-[--color-primary] hover:bg-[--color-primary] hover:text-white transition space-y-3">
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div
          className={`grid gap-3 ${filterFields.length > 2 ? "md:grid-cols-4" : "md:grid-cols-3"}`}
        >
          {filterFields}
      ...
      <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        <Card>
          <h2 className="font-medium">
            {editingId ? `Edit ${meta.title}` : meta.title}
          </h2>
          <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
            ...
            {section === "stock" ? (
              <>
                <Select
                  value={form.itemId || ""}
                  onChange={(event) =>
                    updateField("itemId", event.target.value)
                  }
                  required
                >
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateField("movementDate", event.target.value)
                  }
                />
                <Input
            ...
            {section === "distribution" ? (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Select
                    label="Recipient Type"
                    value={form.recipientType || "other"}
                    onChange={(event) =>
                      updateField("recipientType", event.target.value)
                    }
                  >
                    <option value="class">Class</option>
                    <option value="student">Student</option>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Input
                    ...
                    onChange={(event) =>
                      updateField("distributedAt", event.target.value)
                    }
                  />
                  <Select
                    label="Status"
            ...
            <div className="flex gap-2">
              <Button type="submit" variant="primary" disabled={saving}>
                {saving
                  ? "Saving..."
                  : getCreateButtonLabel(section, editingId)}
              </Button>
          ...
          <div className="mt-4 overflow-x-auto">
            ...
            {!loading && records.length === 0 ? (
              <div className="text-sm text-gray-600">
                No {getRecordLabel(section)} records found.
              </div>
            ) : null}
            {!loading && records.length > 0 ? (
                    ...
                    <tr key={row._id} className="border-t border-gray-100">
                      ...
                      {section === "items" ? (
                        <>
                          ...
                          <td className="py-3 pr-4 text-gray-600">
                            {row.category?.name ||
                              categoryMap.get(row.category)?.name ||
                              "-"}
                          </td>
                          <td className="py-3 pr-4 text-gray-600">

...
export default function ClassRoomsWorkspace({
  roleBase = '/admin',
  ...
  function addRoom() {
    ...
    if (!candidate) return
    if (rooms.some((room) => normalize(room).toLowerCase() === candidate.toLowerCase())) {
  ...
  async function saveRooms() {
    ...
    if (new Set(cleaned.map((room) => room.toLowerCase())).size !== cleaned.length) {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Failed to save room settings')
    } finally {
      setSaving(false)
    }
  }

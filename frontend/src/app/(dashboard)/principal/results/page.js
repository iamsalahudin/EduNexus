...
export default function PrincipalResultsPage() {
  ...
  useEffect(() => {
    ...
    const loadMarksheets = async () => {
      try {
        setLoading(true);
        const result = await marksheetsService.listMarksheets({
  ...
  const handleWeightSave = async (weights) => {
    try {
      setLoading(true);
      setError("");
      const result = await examsService.updateExam(selectedExam._id, {
      ...
      if (result?.exam) {
        ...
        setExams((prev) =>
          prev.map((exam) =>
            exam._id === result.exam._id ? result.exam : exam,
          ),
        );
      }
    } catch (err) {
      setError(
        err?.response?.data?.error || err.message || "Failed to save weights",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 p-4 md:p-6">
      <PageHeader
        ...
        subtitle="Manage marksheets, configure weights, publish results, and export records."
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Exams" value={loading ? "Loading..." : exams.length} />
        <StatCard
          label="Marksheets"
          value={loading ? "Loading..." : marksheets.length}
        />
        <StatCard
          label="Published"
          value={
            loading
              ? "Loading..."
              : marksheets.filter((item) => item.published).length
          }
        />
        <StatCard
          label="Locked"
          value={
            loading
              ? "Loading..."
              : marksheets.filter((item) => item.locked && !item.published)
                  .length
          }
        />
      </div>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
              {exams.map((exam) => (
                ...
                >
                  ...
                  <div
                    className={`text-xs ${selectedExam?._id === exam._id ? "text-slate-200" : "text-slate-500"}`}
                  >
                    {exam.className}
                  </div>
                </button>
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Search by class, section, exam type, or row count.
                      </p>
                    </div>
          ...
          {!selectedExam && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
              <p className="text-slate-600 text-lg">
                Select an exam to view marksheets and manage results
              </p>
            </div>
          )}
        </div>

...
export default function PrincipalTeacherAttendanceDetailPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const [teacherRes, attendanceRes] = await Promise.all([
          ...
          fetchStaffAttendance({ userId: teacherId, fromDate, toDate })
        ])

        if (!mounted) return
        setTeacher(teacherRes?.data?.user || teacherRes?.data || null)
        setRecords(attendanceRes?.records || [])
      } catch (e) {
        if (mounted) setError(e?.response?.data?.error || e.message || 'Failed to load teacher attendance detail')
      } finally {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={teacher?.name ? `Attendance profile for ${teacher.name}` : 'Attendance profile'}
          ...
          <Card className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold">{teacher?.profile?.department || teacher?.profile?.subject || '-'}</p>
              </div>
              <div>

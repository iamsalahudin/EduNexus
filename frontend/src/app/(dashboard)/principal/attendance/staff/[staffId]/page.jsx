...
export default function PrincipalStaffAttendanceDetailPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const [staffRes, attendanceRes] = await Promise.all([
          ...
          fetchStaffAttendance({ userId: staffId, fromDate, toDate })
        ])

        if (!mounted) return
        setStaff(staffRes?.data?.user || staffRes?.data || null)
        setRecords(attendanceRes?.records || [])
      } catch (e) {
        if (mounted) setError(e?.response?.data?.error || e.message || 'Failed to load staff attendance detail')
      } finally {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={staff?.name ? `Attendance profile for ${staff.name}` : 'Attendance profile'}
        right={<ButtonLink href="/principal/attendance/staff" variant="secondary">Back</ButtonLink>}
      />

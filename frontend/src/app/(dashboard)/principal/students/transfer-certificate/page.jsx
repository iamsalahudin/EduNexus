...
export default function TransferCertificatePage() {
  ...
  const [remarks, setRemarks] = useState('No dues pending at the time of issuance.')
  ...
  const certificateData = useMemo(() => ({
    ...
    reason: reason || '-',
    remarks: remarks || '-',
  }), [issueDate, reason, remarks, selectedStudent])
  ...
  async function downloadCertificateById(certificateId, certificateNumber) {
    ...
    link.href = url
    link.download = `${certificateNumber || 'transfer-certificate'}.pdf`
  ...
  async function confirmAndIssue() {
    ...
    try {
      ...
      setError(e?.response?.data?.error || 'Failed to issue and download transfer certificate')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Issue, store, and download officially styled transfer certificates as PDF."
      />
      ...
      <Card>
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-slate-300 bg-gradient-to-br from-slate-50 to-white p-4 sm:p-8 text-sm leading-7 shadow-inner">
          <div className="mx-auto max-w-3xl rounded-lg border-2 border-teal-700 bg-white px-4 sm:px-10 py-6 sm:py-8">
            <div className="mb-6 text-center">
              <div className="mx-auto mb-3 h-14 w-14 rounded-full border-2 border-teal-700 bg-teal-50" />
              <p className="text-xs tracking-[0.22em] text-slate-500">EDUNEXUS INSTITUTIONAL CERTIFICATION</p>
              <h3 className="mt-2 text-2xl sm:text-3xl font-semibold uppercase tracking-wider text-teal-800">Transfer Certificate</h3>
            </div>
            ...
            <div className="mt-6 space-y-2 border-t border-slate-200 pt-4">
              ...
              <p><span className="font-semibold text-slate-800">Class-Section:</span> {certificateData.classSection}</p>
              <p><span className="font-semibold text-slate-800">Issue Date:</span> {certificateData.issueDate}</p>
      ...
      <Card>
        ...
        <p className="text-sm text-gray-600 mt-1">Stored transfer certificates for this selected student.</p>

        <div className="mt-4">
          {historyLoading ? (
            <Skeleton className="h-24" />
          ) : history.length === 0 ? (
            <EmptyState title="No transfer certificates issued yet" />
          ) : (
            <Table>
              <TableRoot>
                <TableHead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <Card className="max-w-md w-full m-4">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Are you sure you want to issue this transfer certificate for <strong>{selectedStudent?.name}</strong>?
            </p>

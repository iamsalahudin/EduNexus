      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                    ...
                    {roleRows.map((row) => (
                      <TableRow key={row.role} className="hover:bg-gray-50">
                        <TableCell className="font-medium">
                          <Link
                            href={`/principal/users/search?role=${encodeURIComponent(row.role)}`}
        ...
        <Card>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-2">
            <ButtonLink href="/principal/users/add" variant="secondary" className="w-full">
              Add New User
            </ButtonLink>
            <ButtonLink href="/principal/users/search" variant="secondary" className="w-full">
              Search Users
            </ButtonLink>
          </div>
                          ...
                          <TableRow key={userId} className="hover:bg-gray-50">
                            ...
                            <TableCell>
                              <div className="flex gap-2">
                                <ButtonLink href={`/principal/users/user/${userId}`} variant="outline" size="sm">
                                  View
                                </ButtonLink>
                                <ButtonLink
                                  href={`/principal/users/update?id=${encodeURIComponent(userId)}`}
                                  variant="outline"
                                  size="sm"
                                >
                                  Edit

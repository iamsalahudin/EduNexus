...
export default function AdminClassLevelsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(e?.response?.data?.error || 'Failed to load level settings')
    } finally {
      setLoading(false)
    }
  }
  ...
  function addLevel() {
    ...
    if (levels.some((lvl) => normalize(lvl) === normalized)) {
  ...
  function updateLevelAt(index, value) {
    setLevels((prev) => prev.map((lvl, i) => (i === index ? value : lvl)))
  }
  ...
  async function saveLevels() {
    ...
    try {
      ...
      setError(e?.response?.data?.error || 'Failed to save level settings')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="View, edit, and add levels used across admin forms."
        actions={<ButtonLink href="/admin/classes">Back to Classes</ButtonLink>}
      />

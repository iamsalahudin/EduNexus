...
export default function EmployeesPage() {
  ...
  async function deleteEmployee(id) {
    ...
    try {
      ...
      setError(e?.response?.data?.error || e.message || 'Failed to delete employee')
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        right={<ButtonLink href="/admin/employees/add">+ Add Employee</ButtonLink>}
      />
                ...
                {filteredEmployees.map((emp) => (
                  <tr key={emp.id || emp._id} className="border-b hover:bg-gray-50 transition">
                    ...
                    <td className="p-3 text-right space-x-2 flex justify-end">
                      <ButtonLink href={`/admin/employees/${emp.id || emp._id}`} variant="secondary" size="sm">
                        Edit
                      </ButtonLink>
                      <Button
                        variant="danger"
                        size="sm"
                        onClick={() => deleteEmployee(emp.id || emp._id)}
                      >
                        Delete
                      </Button>
                    </td>

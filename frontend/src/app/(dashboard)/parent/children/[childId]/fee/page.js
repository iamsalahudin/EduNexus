...
export default function Page({ params }) {
  ...
  useEffect(() => {
    let mounted = true
    setLoading(true)
    fetchFeeDetails(childId || 'child', queryParams).then((res) => {
  ...
  function downloadCsv() {
    ...
    a.href = url
    a.download = `child-fee-${childId || 'report'}.csv`
    a.click()
  ...
  async function downloadXlsx() {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'ChildFee')
      const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' })
      const blob = new Blob([wbout], { type: 'application/octet-stream' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `child-fee-${childId || 'report'}.xlsx`
      a.click()
      URL.revokeObjectURL(url)
    } catch {
      downloadCsv()
  ...
  async function downloadPdf() {
    try {
      ...
      pdf.setFontSize(14)
      pdf.text('Child Fee Report', 40, 40)
      pdf.setFontSize(10)
      let y = 64
      ...
      pdf.save(`child-fee-${childId || 'report'}.pdf`)
    } catch {
      downloadCsv()
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <PageHeader
          ...
          subtitle="View paid and unpaid details for this child."
        />
      ...
      <div className="rounded-lg border bg-white p-4 text-sm text-gray-700">
        Student: <span className="font-medium">{data?.student?.name || '—'}</span>
        {' · '}
        Roll: <span className="font-medium">{data?.student?.roll || '—'}</span>
        {' · '}
        Class: <span className="font-medium">{data?.student?.class || '—'}</span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="font-semibold">
              ...
              <td className="text-right">Rs {monthlyRows.reduce((s, r) => s + Number(r.paid || 0), 0)}</td>
              <td className="text-right">Rs {monthlyRows.reduce((s, r) => s + Number(r.due || 0), 0)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="rounded-lg border bg-white p-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="font-semibold">
              <td className="py-2">Total</td>
              <td className="text-right">Rs {yearlyRows.reduce((s, r) => s + Number(r.paid || 0), 0)}</td>
              <td className="text-right">Rs {yearlyRows.reduce((s, r) => s + Number(r.due || 0), 0)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="rounded-lg border bg-white p-4">
        <h3 className="font-medium mb-3">Paid and Unpaid Fee History</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={row.voucherNo} className="border-b last:border-none">

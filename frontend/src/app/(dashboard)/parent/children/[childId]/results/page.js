  ...
  useEffect(() => {
    const loadResults = async () => {
      try {
        ...
        // Extract child info from first result
        if (result.reportCards?.length > 0) {
          setChildInfo(result.reportCards[0].student);
        }
      } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">
            {childInfo?.user?.name}'s Results
          </h1>
          <p className="text-gray-600 mt-1">View published exam results for your child</p>
        </div>
